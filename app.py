...
def set_df_styles(results: pd.DataFrame, bg_color: str, font_color: str):
    return (
        results.style
        .set_properties(**{"background-color": bg_color, "color": font_color})
    )
...
def get_attributes_to_filter_df(results: pd.DataFrame, key: str) -> tuple:
    ...
    is_dark_theme = st.checkbox("Want Dataframe in Dark Theme?", False, key=key + '_df_style')
...
@st.cache_data
def get_filtered_df(results: pd.DataFrame, no_of_rows_to_show: int, columns_to_show: list) -> pd.DataFrame:
...
no_of_rows_to_show, columns_to_show, is_dark_theme = get_attributes_to_filter_df(netflix_data, key="netflix_data")
